...
function buildSeedDossier(quizResults) {
  const q = quizResults || {};
  const list = /* @__PURE__ */ __name((arr) => Array.isArray(arr) && arr.length ? arr.join(", ") : "(unknown)", "list");
  const single = /* @__PURE__ */ __name((v) => v === null || v === void 0 || v === "" ? "(unknown)" : String(v), "single");
  return [
...
var EMAIL_RE, DOSSIER_VERSION_MARKER, DOSSIER_MAX_CHARS, EXCHANGE_RESET_AT, DOSSIER_PREFIX, CHAT_PREFIX, GEMINI_API_BASE, GEMINI_SAFETY_SETTINGS, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_FALLBACK_MODEL, DEPRECATED_GEMINI_MODEL_RE;
var init_lib = __esm({
  "_lib.js"() {
...
var RETRYABLE_STATUS, RETRY_DELAYS_MS, MAX_OUTPUT_TOKENS_CAP;
...
function deriveSkillGapEntries(tree, waypoint) {
  ...
  const add = /* @__PURE__ */ __name((label, source) => {
...
function normalizeV3Gap(raw) {
  ...
  const clampScore3 = /* @__PURE__ */ __name((n) => Math.max(0, Math.min(100, Math.round(Number(n) || 0))), "clampScore");
...
var ROADMAP_TREE_VERSION, MAX_TREE_NODES, MAX_TREE_DECISIONS, MAX_TREE_DEPTH, ROADMAP_TREE_MAX_CHARS, SPINE_WAYPOINT_COUNT, MAJOR_WAYPOINT_COUNT, MAX_STEPS_PER_NODE, MAX_STEP_TEXT, CAREER_VALUES, ACTION_TYPES, NODE_TYPES, PATH_ROLES, HORIZONS, SLUG_RE, nodeIdCounter, KEYWORD_PROGRESS_PER_MATCH, MAX_CHECKLIST_ITEMS, MAX_CHECKLIST_TEXT, MAX_V3_GAPS, COORDINATE_GAP_SOURCE, MAX_BRANCH_FOCUSES, MAX_EXTEND_NODES;
...
var SCHEMA_ID, DIM_COUNT, MAX_SOC_BATCH, GEMINI_CAREER_BATCH_DISABLED, STATIC_ARTIFACT_BASE, SIMILARITY_ADJACENT, SIMILARITY_CROSS_SECTOR;
...
__export(store_exports, {
  DIM_COUNT: () => DIM_COUNT,
  SCHEMA_ID: () => SCHEMA_ID,
  clearOnetCache: () => clearOnetCache,
  getCareers: () => getCareers,
  getDerivedCareers: () => getDerivedCareers,
  getImBuffer: () => getImBuffer,
  getLayout: () => getLayout,
  getLvBuffer: () => getLvBuffer,
  getManifest: () => getManifest,
  getRegistry: () => getRegistry,
  getSimilarityIndex: () => getSimilarityIndex,
  getSocIndex: () => getSocIndex,
  getZoneDimensionProfiles: () => getZoneDimensionProfiles,
  sliceVector: () => sliceVector,
  validateSocList: () => validateSocList
});
...
var manifestCache, registryCache, careersCache, similarityCache, layoutCache, socIndexCache, lvBufferCache, imBufferCache, zoneProfilesCache, derivedCareersCache, derivedStaticCache, derivedD1Cache, derivedD1CacheAt, DERIVED_D1_TTL_MS, careersDerivedSocs;
...
__export(career_lookup_exports, {
  ...
  buildProposalReply: () => buildProposalReply,
  extractCareerPhraseFromMessage: () => extractCareerPhraseFromMessage,
  fuzzySearchOnetCareers: () => fuzzySearchOnetCareers,
  isAffirmativeConfirmation: () => isAffirmativeConfirmation,
  isNegativeConfirmation: () => isNegativeConfirmation,
  lookupOnetCareerInMessage: () => lookupOnetCareerInMessage,
  proposeOnetCareersForMessage: () => proposeOnetCareersForMessage,
  resolveProposalChoice: () => resolveProposalChoice,
  searchOnetCareersByTitle: () => searchOnetCareersByTitle,
  validateOnetCareer: () => validateOnetCareer
});
...
var MARK_DONE_RE, ROADMAP_VERSION, ROADMAP_MAX_CHARS, ROADMAP_FRESH_MS, PHASE_DEFS, ACTION_TYPES2, SLUG_RE2, actionIdCounter, CAREER_PIVOT_RULES, MAX_ROADMAP_MSG_LEN, MAX_ROADMAP_HISTORY;
var init_roadmap = __esm({
  "_lib/roadmap.js"() {
    ...
    __name(normalizeActionType2, "normalizeActionType");
...
__export(auth_exports, {
  ANALYSIS_FRESH_MS: () => ANALYSIS_FRESH_MS,
  MIN_PASSWORD_LEN: () => MIN_PASSWORD_LEN,
  RATE_LIMIT_ANALYSIS_MAX: () => RATE_LIMIT_ANALYSIS_MAX,
  RATE_LIMIT_DERIVE_MAX: () => RATE_LIMIT_DERIVE_MAX,
  RATE_LIMIT_GAP_CHECKLIST_MAX: () => RATE_LIMIT_GAP_CHECKLIST_MAX,
  RATE_LIMIT_MAX: () => RATE_LIMIT_MAX,
  RATE_LIMIT_SPLIT_MAX: () => RATE_LIMIT_SPLIT_MAX,
  RATE_LIMIT_SYNC_MAX: () => RATE_LIMIT_SYNC_MAX,
  RATE_LIMIT_WINDOW_SEC: () => RATE_LIMIT_WINDOW_SEC,
  RESET_TOKEN_TTL_SEC: () => RESET_TOKEN_TTL_SEC,
  SESSION_COOKIE: () => SESSION_COOKIE,
  SESSION_DAYS: () => SESSION_DAYS,
  authErrorResponse: () => authErrorResponse,
  authJsonResponse: () => authJsonResponse,
  authPreflight: () => authPreflight,
  checkRateLimit: () => checkRateLimit,
  clearSessionCookieHeader: () => clearSessionCookieHeader,
  clientIp: () => clientIp,
  ...
  invalidateCareerAnalyses: () => invalidateCareerAnalyses,
  isValidPassword: () => isValidPassword,
  loadCareerAnalyses: () => loadCareerAnalyses,
  loadCareerAnalysis: () => loadCareerAnalysis,
  loadQuizProfile: () => loadQuizProfile,
  loadRoadmap: () => loadRoadmap,
  optionalSession: () => optionalSession,
  parseCookies: () => parseCookies,
  quizProfileToSeed: () => quizProfileToSeed,
  requireSession: () => requireSession,
  saveCareerAnalysis: () => saveCareerAnalysis,
  saveQuizPortalSnapshot: () => saveQuizPortalSnapshot,
  saveQuizProfile: () => saveQuizProfile2,
...
var SESSION_COOKIE, SESSION_DAYS, MIN_PASSWORD_LEN, RATE_LIMIT_MAX, RATE_LIMIT_ANALYSIS_MAX, RATE_LIMIT_SYNC_MAX, RATE_LIMIT_SPLIT_MAX, RATE_LIMIT_DERIVE_MAX, RATE_LIMIT_GAP_CHECKLIST_MAX, RATE_LIMIT_WINDOW_SEC, RESET_TOKEN_TTL_SEC, ANALYSIS_FRESH_MS, ANALYSIS_MAX_CHARS, PBKDF2_ITERATIONS;
var init_auth = __esm({
  "_lib/auth.js"() {
    init_functionsRoutes_0_40739639759313073();
    init_lib();
    ...
    __name(saveQuizProfile2, "saveQuizProfile");
...
var SECTOR_KEYS, SECTOR_TO_ZONE, PERSONALITY_OBJECTIVE_BLEED;
...
var MAX_DELTA_PER_DIM, MAX_RULES_LAYER, PROFILE_BOOST_SCALE, PROFILE_WEIGHT_FLOOR, DOMINANT_ZONE_COUNT, NON_DOMINANT_ATTENUATION, profilesCache, FALLBACK_PROFILES;
...
var init_refine_map = __esm({
  "_lib/onet/refine-map.js"() {
    ...
    __name(emptyVector2, "emptyVector");
...
var CONTEXT_MAX, RESUME_EXCERPT_MAX, RATE_LIMIT_OBJECTIVE_PATCH_MAX, MAX_PATCH_DIMENSIONS, DOSSIER_EXCERPT_LEARNING_MAX;
...
var MAX_ANSWER_LEN, MAX_CUSTOM_ANSWERS;
...
var SALARY, TECH_SCORE, SLUG_INDUSTRY, HUB_ZONE_TO_INDUSTRY, JOB_ZONE_EDUCATION;
...
var init_dossier_parse = __esm({
  "_lib/dossier-parse.js"() {
    ...
    __name(patchDossierLine2, "patchDossierLine");
...
var PROPOSAL_KV_PREFIX, PROPOSAL_TTL_SEC, RATE_LIMIT_PROPOSE_MAX;
...
var CONTEXT_MAX2, DOSSIER_EXCERPT_MAX, RATE_LIMIT_PERSONALITY_PATCH_MAX, MIN_VARIANCE;
...
var SECTOR_KEYS2, SHEET_VERSION, HISTORY_MAX, MAX_DELTA_PER_PATCH;
...
var KNOW_YOU_MAX, CHARACTER_MAX, NOTE_MAX, DOSSIER_MAX, PENDING_ANALYSIS_MSG;
...
var init_roadmap_vector_fit = __esm({
  "_lib/onet/roadmap-vector-fit.js"() {
    ...
    init_math();
    init_user_vectors();
    __name(loadZoneCentroids2, "loadZoneCentroids");
...
__export(focus_keywords_exports, {
  KEYWORD_PROGRESS_PER_MATCH: () => KEYWORD_PROGRESS_PER_MATCH2,
  buildFallbackKeywords: () => buildFallbackKeywords,
  enrichFocusTrackerKeywords: () => enrichFocusTrackerKeywords,
  enrichGapKeywords: () => enrichGapKeywords
});
function buildFallbackKeywords(gapLabel, stepTexts = []) {
  const tokens = /* @__PURE__ */ new Set();
  const addText = /* @__PURE__ */ __name((text) => {
...
async function enrichFocusTrackerKeywords(tree, env, opts = {}) {
  ...
  const gapNeedsKeywords = /* @__PURE__ */ __name((g) => g.source !== "coordinate" && (!Array.isArray(g.keywords) || !g.keywords.length || opts.upgrade && g.keywordsSource === "fast"), "gapNeedsKeywords");
  ...
  const enrichOne = /* @__PURE__ */ __name(async (gap) => {
...
var STOP_WORDS, KEYWORD_PROGRESS_PER_MATCH2;
...
var MAX_DOSSIER_LEN, MAX_MSG_LEN, SPINE_RETRY_APPEND;
...
var SLUG_RE3, STRICT_CATALOG_SOURCES, FOCUS_WEIGHTS, SLUG_ALIASES, RETARGET_WEIGHT_MIN, MAX_FOCUS_HISTORY, SWITCH_WINDOW_DAYS;
...
function sanitizeVectorDimensions(raw) {
  if (!raw || typeof raw !== "object") return null;
  const clampScore3 = /* @__PURE__ */ __name((n) => Math.max(0, Math.min(100, Math.round(Number(n) || 0))), "clampScore");
  const clampSigned = /* @__PURE__ */ __name((n) => Math.max(-100, Math.min(100, Math.round(Number(n) || 0))), "clampSigned");
  const packOne = /* @__PURE__ */ __name((item) => {
  ...
  const packList = /* @__PURE__ */ __name((list) => Array.isArray(list) ? list.slice(0, 8).map(packOne).filter(Boolean) : [], "packList");
...
function formatVectorDimensionsBlock(vectorDimensions) {
  ...
  const line = /* @__PURE__ */ __name((d) => `- ${d.name}${d.domain ? ` (${d.domain})` : ""}: you ${d.user} vs role ${d.target}`, "line");
...
var SLUG_RE4, MAX_SLUG_LEN, MAX_NAME_LEN, MAX_DOSSIER_LEN2, MAX_MSG_LEN2, MAX_HISTORY, CAREER_CHAT_PREFIX, RETRYABLE_GEMINI_STATUS, GEMINI_RETRY_DELAYS_MS, AI_SUMMARY_MAX_WORDS, AI_SUMMARY_MAX_CHARS, AI_INSIGHT_MAX_WORDS, AI_INSIGHT_MAX_CHARS, AI_CONSIDERATION_MAX_WORDS, AI_CONSIDERATION_MAX_CHARS;
var init_career_analysis = __esm({
  "career-analysis.js"() {
    ...
    __name(trimForPrompt2, "trimForPrompt");
    __name(trimProse, "trimProse");
    __name(formatQuizScoresForPrompt2, "formatQuizScoresForPrompt");
    __name(parseJsonFromText2, "parseJsonFromText");
...
async function nextSyntheticSoc(env) {
  ...
  if (!maxSeq) return { seq: 1e3, format: /* @__PURE__ */ __name((n) => `99-${n}.00`, "format") };
  const m = String(maxSeq).match(/^99-(\d+)\.00$/);
  const start = m ? Number(m[1]) + 1 : 1e3;
  return { seq: start, format: /* @__PURE__ */ __name((n) => `99-${n}.00`, "format") };
}
...
var SOC_RE, SLUG_RE5, MAX_SLUG_LEN2, MAX_NAME_LEN2;
...
function sanitizeChecklistGaps(rawGaps) {
  ...
  for (const g of rawGaps) {
    ...
    if (!name) continue;
    const clamp = /* @__PURE__ */ __name((n) => Math.max(0, Math.min(100, Math.round(Number(n) || 0))), "clamp");
    seen.add(dimIndex);
    out.push({
      dimIndex,
      name,
...
var SLUG_RE6, MAX_SLUG_LEN3, MAX_NAME_LEN3, MAX_DOSSIER_LEN3, MAX_MSG_LEN3, ROADMAP_CHAT_PREFIX, SOC_RE2, GAP_CHECKLIST_TTL, MAX_CHECKLIST_ACTIONS, MAX_CHECKLIST_ACTION_CHARS;
var init_career_roadmap = __esm({
  "career-roadmap.js"() {
    ...
    __name(trimForPrompt3, "trimForPrompt");
    __name(isValidQuizScores2, "isValidQuizScores");
    __name(resolveDossier2, "resolveDossier");
...
var MAX_MESSAGE_CHARS, RETRYABLE_STATUS2, RETRY_DELAYS_MS2;
var init_chat = __esm({
  "chat.js"() {
    init_functionsRoutes_0_40739639759313073();
    init_lib();
...
async function onRequestPost9(context) {
  ...
  try {
    ...
    if (stage === "feedback") {
      ...
      const axis = /* @__PURE__ */ __name((o) => ({ score: clampScore2(o && o.score), note: clampStr(o && o.note, 240) }), "axis");
      const feedback = {
...
var BANK_TTL, SEEN_TTL, MAX_ANSWER, TYPES2;
...
async function warmFragmentsForSnapshot(env, context, { quiz, careerPool, email, baseUrl }) {
  try {
    const candidates = [];
    const seen = /* @__PURE__ */ new Set();
    const pushSoc = /* @__PURE__ */ __name((soc) => {
...
var FALLBACK_FRESH_MS, SOC_RE4, MAX_WARMUP_CANDIDATES;
...
var MAX_RESUME_CHARS, MAX_NAME_LEN4, INDUSTRY_KEYS;
var init_quiz_enrich = __esm({
  "quiz-enrich.js"() {
    ...
    __name(parseJsonFromText3, "parseJsonFromText");
    __name(callGemini4, "callGemini");
...
var CACHE_TTL, TOP_DIMS, EVIDENCE;
var init_resume_builder = __esm({
  "resume-builder.js"() {
    ...
    __name(clampStr2, "clampStr");
    __name(dailyLimit2, "dailyLimit");
...
var RETRYABLE_STATUS3, RETRY_DELAYS_MS3, MIN_RESUME_CHARS, MAX_RAW_EXTRACT_CHARS, PHASE1_PROMPT, PHASE2_PROMPT_PREFIX, PHASE2_DIRECT_PDF_PROMPT;
...
var MAX_RESUME_CHARS2, MAX_NAME_LEN5, MAX_BASE64_CHARS, INDUSTRY_KEYS2, INDUSTRY_KEYWORDS;
var init_resume_parse = __esm({
  "resume-parse.js"() {
    ...
    __name(normalizeBoosts2, "normalizeBoosts");
...
var RESEND_ENDPOINT2, EMAIL_RE2, RESEND_FETCH_TIMEOUT_MS;
...
var MAX_MESSAGES, MAX_MSG_CHARS, MAX_USER_MSGS, RATE_MAX;
...
var init_sim_feedback = __esm({
  "sim-feedback.js"() {
    ...
    MAX_ARTIFACT = 6e3;
    RATE_MAX2 = 12;
    __name(loadContext2, "loadContext");
...
var SLUG_RE7, MAX_SLUG_LEN4, MAX_NAME_LEN6, RATE_MAX3, KV_TTL_SECONDS;
var init_sim_generate = __esm({
  "sim-generate.js"() {
    ...
    __name(buildPrompt2, "buildPrompt");
    __name(buildCriticPrompt, "buildCriticPrompt");
    __name(validate, "validate");
    __name(generateOnce2, "generateOnce");
...
var ALLOWED_TIERS, RATE_LIMIT_MAX2;
...
var KV_TTL_SEC, RATE_LIMIT_MAX3;
...
async function onRequest20(context) {
  ...
  try {
    ...
    const explanations = {};
    const misses = [];
    const cacheKeyFor = /* @__PURE__ */ __name((it) => `stretch:${email}:${it.slug}:${hashKey(it.drivers.join("|") + "|" + it.soc)}`, "cacheKeyFor");
    if (kv) {
...
var MAX_ITEMS, MAX_WORDS, CACHE_TTL_SEC, MAX_DRIVERS;
...
var init_vector_fit = __esm({
  "vector-fit.js"() {
    ...
    init_user_vectors();
    init_constants();
    __name(loadZoneCentroids3, "loadZoneCentroids");
...
// ../.wrangler/tmp/bundle-ZD7K8D/middleware-loader.entry.ts
init_functionsRoutes_0_40739639759313073();

// ../.wrangler/tmp/bundle-ZD7K8D/middleware-insertion-facade.js
init_functionsRoutes_0_40739639759313073();
...
//# sourceMappingURL=functionsWorker-0.4365665557643674.mjs.map
